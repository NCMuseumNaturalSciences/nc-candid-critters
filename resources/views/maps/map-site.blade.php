    ...
    <div id="map-container">
        ...
        <div id="map">
            ...
            <div id="map-top-bar" class="map-top-bar container-fluid">
                <div class="map-title text-left">Deployment Name: {{ $site->site_name }}</div>
            </div>
        </div>
    </div>
    @include('modals.site-info-dialog')
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $("#site-info-dialog").dialog({
                ...
                width: 768,
                responsive: true,
                position: {my: "left top", at: "left bottom", of: "#map"},
                buttons: [
                    {
                        text: "Close",
                        click: function () {
            ...
            function onEachFeature(feature, layer) {
                ...
                var available_boolean = layer.feature.properties.available_yn;
                var available_yn;
                if(available_boolean == 1) {
                    available_yn = 'Available'
                }
                else {
                    available_yn = 'Not Available'
                }
                ...
                layer.on('click', function (e) {
                    ...
                    $('.marker-long').html(layer.feature.properties.long);

                    $('li.lat span').html(layer.feature.properties.lat);
                    $('li.long span').html(layer.feature.properties.long);
                    $('li.habitat span').html(layer.feature.properties.habitat);
                    $('li.property_name span').html(layer.feature.properties.property_name);
                    $('li.availability span').html(available_yn);

                    $('a.infowin-zoombtn').attr('href','');
                    $('#site-info-dialog').dialog('option', 'title', 'Deployment: ' + layer.feature.properties.site_name);
            ...
            function createMap(data) {
                ...
                var layerBounds = siteLayer.getBounds();
                map.fitBounds(layerBounds, {padding: [0, 0]});
            };
